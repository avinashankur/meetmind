...
For each changed file, read enough to understand _what_ the change is about.
...
Build a mental model of each change's _purpose_ — what feature, fix, chore,
...
4. **Generate the commit message using the `git-commit` skill.**
   ...
   _Fallback (only if `git-commit` skill is not available in the workspace):_
   - Subject: `<type>(<scope>): <imperative summary>` (≤50 chars, max 72, no trailing period).
   - Body: Add only when _why_ isn't obvious, for breaking changes, or when bundling coupled elements (bullets `- ...` wrapped at 72 chars).
